  ...
  angriModule.directive('angrid',  function($compile) {
    return {
      compile: function compile(element) {
        // console.log(element.children('tbody').children('tr').attr('data-ng-repeat'));
        var tr = element.children('tbody').children('tr');
        var ngRepeatAttr = null;
        if (tr.attr('ng-repeat')) {
          ngRepeatAttr = 'ng-repeat';
        } else if (tr.attr('data-ng-repeat')) {
          ngRepeatAttr = 'data-ng-repeat';
        } else {
         console.warn("Could not find the ng-repeat attribute.")
        }

        var index             = 0,
            sourceExpression  = tr.attr(ngRepeatAttr).match(/^\s*(.+)\s+in\s+(.*)\s*$/),
            baseExpression    = sourceExpression[2],
            itemExpression    = sourceExpression[1];

        tr.attr(ngRepeatAttr, itemExpression + ' in angri.filteredList() | angri_filter_skip:angri.page:angri.limit | orderBy:angri.predicate:angri.reverse | limitTo:angri.limit | angri_filter_forceLimit');
